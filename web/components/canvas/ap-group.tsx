...
export function APPredict(props: { dataset: Data[] }) {
  const [params, setParams] = useState<Params>([0, 0, 1, 1]);

  useEffect(() => {
    const interval = setInterval(() => {
      const out = step(props.dataset, params, 0.01);
      setParams(out);
      console.log(out);
    }, 20);
  ...
  return (
    <group>
      {props.dataset.map(([x, y, z], i) => {
        ...
        return (
          <Box key={i} position={[x, h / 2, y]} scale={[scale, h, scale]}>
      ...
      {array.map((x) => {
        return array.map((y) => {
          ...
          return (
            <Box key={y * 1000 + x} position={[x, h, y]} scale={0.5}>
